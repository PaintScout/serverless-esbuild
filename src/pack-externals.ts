...
function getProdModules(externalModules: { external: string }[], packageJsonPath: string) {
  ...
  forEach(externalModule => {
    ...
    console.log(externalModule, moduleVersion);
    if (moduleVersion) {
...
export async function packExternalModules(this: EsbuildPlugin) {
  ...
  // (if this is not a monorepo, it will be the same as rootPackageJsonPath)
  ...
  // if this is a monorepo, packageJson will for this project, otherwise it will be rootPackageJson
  const packageJson: Record<string, any> = isMonorepo
  ...
  const relativePath = path.relative(compositeModulePath, path.dirname(rootPackageJsonPath));
  ...
  await packager.install(compositeModulePath, isMonorepo);
